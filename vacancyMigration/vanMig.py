...
number_per_layer = 100  # 每层最多100个原子
...
class vacancyMigration1(object):
    def __init__(self, cifFile=' '):
        ...
        # 得到所有阵点的坐标t(index --> atoms的编号), 并转化成list形式
        # 95个sites
        self.sites_position = list(list(x)
...
# 一、 原子分层
    ...
    def get_layers(self):
        ...
        self.sitesLabel_per_layer = [[] for i in range(len(self.z_lst))]
        for i in self.sites_position:
            for j in self.z_lst:
                if (epsilon_criterion(x=i[2], standard=j, epsilon=epsilon)):
                    '''
                    ...
                    print("编号为 {0} 的原子属于 {1} 层".format(
                        self.sites_position.index(i), self.z_lst.index(j)))
                    '''
                    self.sitesLabel_per_layer[self.z_lst.index(j)].append(
                        self.sites_position.index(i))
            ...
            for j in range(len(self.z_lst)):
                if (i < j):
                    minus_vector = vector_minus(
                        self.sites_position[self.sitesLabel_per_layer[i][0]], self.sites_position[self.sitesLabel_per_layer[j][0]])
...
# 二、 找出空位可能扩散的位点
    ...
    def get_diffusion_layers(self):
        ...
        self.layer3 = self.sitesLabel_per_layer[3]     # layer: 16个原子
        self.layer4 = self.sitesLabel_per_layer[4]     # layer: 15个原子
    ...
    def get_nearest_sites_sameplane(self, center=57):
        ...
        # (2.1) 在layer3/layer4中，以某一原子找出相邻原子的大致距离
...
if __name__ == '__main__':
    motion = vacancyMigration1(cifFile='/Users/mac/Desktop/空位扩散/CONTCAR.cif')
    # motion.get_nearest_sites_sameplane(center=57)
    print("空位最后的位置是：", motion.migration_nsteps(center=57, nsteps=5))
    print("10次扩散5步生成10种结构，空位的位置分别为：\n", motion.n_migrations(center=57, nsteps=5, count=10))